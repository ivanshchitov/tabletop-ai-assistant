...
- **Внутри себя содержит фиксированный системный промпт** — единая строка, объединяющая роль
  ассистента и логику работы (фильтрация тем, формат ответов) — жёстко зашит в код, на экране не
  показывается.
...
## ВСТРОЕННЫЙ ПРОМПТ (HARDCODED) — переносится без изменений

Живёт в `feature/assistant/domain/AssistantPrompts.kt` как единая константа `SYSTEM_PROMPT` (роль
ассистента + логика поведения и фильтрации объединены в одну строку). Это инструкция для LLM, а не
UI-текст — она **не локализуется** и не выносится в строковые ресурсы, даже несмотря на общее требование
локализации UI.

### СИСТЕМНЫЙ ПРОМПТ (роль + поведение + фильтрация)
```
...
Отправка: `system = SYSTEM_PROMPT`, `user = "Вопрос пользователя: <текст>"`
(см. `AssistantPrompts.buildUserPrompt`).
...
│   │   ├── domain/
│   │   │   ├── AssistantPrompts.kt # SYSTEM_PROMPT (не локализуется)
...
2. **Отправка вопроса**:
   ...
   - Сообщение пользователя добавляется в список, `isSending = true`, отправляется запрос с единым
     системным промптом.
...
| **Системный промпт** | Единая строка, жёстко зашита в коде (роль эксперта + фильтрация тем + формат ответов, требование Markdown), не локализуется |
